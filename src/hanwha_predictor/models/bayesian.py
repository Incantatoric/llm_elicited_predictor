...
class HanwhaBayesianModel:
    ...
    def predict(self, X_new: np.ndarray, n_samples: int = 1000) -> Dict[str, np.ndarray]:
        ...
        # Create new model with updated data for prediction
        import pymc as pm
        ...
        with pm.Model() as pred_model:
            # Recreate the same model structure but with new X data
            mixture_weights = np.ones(n_priors) / n_priors
            
            # Create mixture priors for each parameter (same as training)
            betas = []
            for param_idx in range(n_features + 1):  # +1 for intercept
                means = self.priors[:, param_idx, 0]
                stds = self.priors[:, param_idx, 1]
            ...
            # Linear regression equation with NEW data
            intercept = betas[0]
            feature_betas = pm.math.stack(betas[1:])
            mu = intercept + pm.math.dot(X_new, feature_betas)
            
            # Noise term
            sigma = pm.HalfCauchy('sigma', beta=1.0)
            
            # Likelihood (this will be sampled)
            y_pred = pm.Normal('y_obs', mu=mu, sigma=sigma)
            
            # Sample from posterior predictive using the trained trace
        ...
        predictions = {
            'mean': posterior_predictive['y_obs'].mean(axis=(0,1)),
            'std': posterior_predictive['y_obs'].std(axis=(0,1)),
            'samples': posterior_predictive['y_obs'],
            'quantiles': {
                '5%': np.percentile(posterior_predictive['y_obs'], 5, axis=(0,1)),
                '25%': np.percentile(posterior_predictive['y_obs'], 25, axis=(0,1)),
                '75%': np.percentile(posterior_predictive['y_obs'], 75, axis=(0,1)),
                '95%': np.percentile(posterior_predictive['y_obs'], 95, axis=(0,1))
            }
        }
